...
> 2026-05-20 对齐注记：本文中的 “Embed Orchestra” 已拆分为 **Telegraph Native Harness**
> 与其底层 **Embedded Execution Kernel**；spawn CLI 则升级为 **External Agent Runtime**
> 一等产品路径。详见 [D-015](./20260520-agent-runtime-product-layer-alignment.md)。
...
## 6. 混合模式与演进路径（2026-05-20 更新）

两种模式并非互斥。Telegraph 的 A-005 已经明确了混合策略：

- **External Agent Runtime**：Pi CLI / Codex CLI / Claude Code 等成熟 CLI 是一等路径，用于兼容上游产品自己的 agent、extension、config 与目录约定。
- **Telegraph Native Harness**：Telegraph 自己定义 agent profile、subagent、context policy、ToolRegistry、Permission 与 Trace。
- **Embedded Execution Kernel**：只作为 Native Harness 的底层执行内核，负责 model loop / tool loop / event bridge；它不是用户直接选择的产品层。
...
阶段 2: 结构化双向协议（ACP / Pi-RPC）
         ↓
阶段 3: Embedded Execution Kernel（Native Harness 的 model/tool loop）
         ↓
阶段 4: Telegraph Native Harness（agent profile / subagents / permission / trace）
```
...
- **External Agent Runtime 是"广度优先"策略**：最大化 agent 覆盖范围，最小化集成成本，代价是可观测性和控制力。
- **Telegraph Native Harness 是"深度优先"策略**：最大化可观测性、可控性和用户体验，代价是集成成本和维护范围。
- 两种模式对应不同的产品定位：open-design 是"agent 调度器"，Telegraph 是"agent 工作台"。
- 在实践中，保留 External Agent Runtime 作为成熟 CLI 产品路径，同时建设 Telegraph Native Harness；Embedded Execution Kernel 只是后者的底层，不再作为独立产品路线。
