...
> **⚠ 2026-05-15 更新**：此条描述有误。`UtilityProcessSupervisor.restart()` 内部已自动调用
> `orch.replaceParticipantChannel(participantId, newChannel, { autoReconnect: true })`
> + `channel.setKillOnDisconnect(false)`。telegraph 的 `PageletProcess` / `DaemonProcess`
> 通过 supervisor 间接使用了 `replaceParticipantChannel`，只是没有在 telegraph 代码中
> **直接**调用。supervisor restart 的透明换链路径已完整。
...
> **⚠ 2026-05-15 更新**：✅ **已交付**。x-oasis 0.16.0 提供 `ReconnectPolicySpec` +
> `ConnectionConfigSpec` + `instantiateReconnectPolicy()`；telegraph 已升依赖到 0.16.0 /
> 0.13.0，`PageletWorker.connectPeer()` 现在把 `peerConnectionConfig(peerLabel)` 传给
> `proxy.connect()`。worker 可通过 override `peerConnectionConfig()` 配置 per-pagelet
> 重连策略；`defaultConnectionConfigSpec()` 辅助函数提供与 main 侧一致的默认参数。

~~pagelet→shared/daemon 的重连节奏（max retries / backoff cap）完全用 main 侧默认值。
没法按 pagelet 维度调整。~~

**前置依赖**：~~D-007 G7 在 x-oasis 上游解决。~~ ✅ 已解决。
...
> **⚠ 2026-05-15 更新**：G1/G7 已不再阻塞。G2 调用级接入经评估决定不做（连接级已接入）。

| Gap | x-oasis 任务 | 阻塞 telegraph 侧 | 当前状态 |
|-----|--------------|-------------------|---------|
| ~~G1~~ | ~~supervisor `restart()` 走 `bindPort({rebind:true}) + replaceParticipantChannel`~~ | ~~H1（透明换链）~~ | ✅ **已交付** — supervisor restart 自动走 `replaceParticipantChannel`，telegraph 间接使用 |
| G2 | CircuitBreaker 接入 RPC 调用栈 | H2 forwardingProxy 终极版 | 🟡 **连接级已接**，调用级经评估决定不做 |
| ~~G7~~ | ~~`ReconnectPolicy` 跨进程序列化~~ | ~~H6（pagelet 维度调整重连节奏）~~ | ✅ **已交付** — `ReconnectPolicySpec` + `ConnectionConfigSpec`，x-oasis 0.16.0，telegraph `PageletWorker` 已传 spec |
