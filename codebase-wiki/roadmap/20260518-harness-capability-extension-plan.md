...
description: >
  ...
  shell、filesystem、patch 等 integration；Pi extension 生态改由 Pi CLI External Agent Runtime 承载。
category: roadmap
created: 2026-05-18
updated: 2026-05-21
tags:
...
- `coding` 默认围绕 workspace、repo status、patch apply、terminal feedback 等能力组织；第三方 agent 产品能力应优先通过 External Agent Runtime 托管，而不是塞进 native harness core。
...
- Pi extension compatibility 不进入 harness core。2026-05-21 后，旧 `piExtensionCompatProfile()` / `PiExtensionCompatHost` 已删除；Pi extension 生态应通过 Pi CLI External Agent Runtime 运行，或通过 importer 转换为 Telegraph native profile。
...
## 历史 Pi Inline Bash 示例映射

以下是已删除 `pi-compat` shim 曾经尝试兼容的 Pi extension 形态；现在只作为历史背景，不再是 native harness 的目标能力：
...
|------|--------|----------|-------------------|----------|
| Pagelet base capability | `chatCapabilities()` | prompt preprocess、轻量 feedback、chat-local tools、chat context | 可启用 readonly workspace、shell automation、coding edit，但必须有 run profile 与 permission | 隐式 shell、隐式 filesystem write |
...
| Pagelet/mode base capability | `codingCapabilities()` | workspace context、terminal feedback、repo-aware tools、diff/patch workflow | 可启用 shell、filesystem、patch apply | design canvas 状态模型 |
...
这里可以把 shell/filesystem/patch 理解为 integration：它们把外部系统接进 Telegraph harness。但它们不是协议层，也不是 pagelet 的天然身份。`AgentEvent` 只负责把结果、权限、trace 表达成可序列化事实；integration adapter 负责执行和安全边界。Pi extension 生态不再通过 native compatibility shim 接入，改由 Pi CLI External Agent Runtime 承载。
...
- [x] chat/design base pack 默认不启用 shell/filesystem write，但允许通过 explicit task profile 临时启用 shell/filesystem/patch integration。

### Phase 4：Pi Extension Compatibility（已删除）

- [x] 2026-05-21 架构收敛：删除旧 `packages/agent/src/extensions/pi-compat`，不再维护伪 Pi Extension API。
- [x] Pi CLI / Pi extensions / pi-subagents 归属 External Agent Runtime；Telegraph Native Harness 只维护 Telegraph-owned profile、tool、permission 与 trace。

### Phase 5：Product Wiring

- [x] 为 future coding pagelet 或 coding mode 启用 `codingCapabilities() + shell/filesystem/patch integration`。（2026-05-21 已删除 Pi compat shim；只有 explicit `shell-automation` / `coding-edit` / `design-build` 才按 run 注入 process/filesystem/patch，权限不跨 run 泄漏）
...
- [x] Trace panel 按 run 展示 extension hook、permission、exec、feedback。（2026-05-21 LLM Trace timeline 继续支持 generic hook/source 摘要，不再保留 Pi compat 专门分支）
...
- Chat/design tests：默认没有 shell/filesystem capability；带 explicit profile 时可以请求 shell/filesystem/patch，并被 PermissionBroker 正确约束。
...
- coding pack 可运行 Telegraph-owned shell/filesystem/patch integration；Pi extension 行为交给 Pi CLI External Agent Runtime。
...
- design 是否只支持 native design capability，还是未来需要独立的 external-runtime bridge？
