...
description: >
  ...
  Slack、MCP、Computer Use、Relay 与 Sandbox 的能力取舍、技术象限、
  分阶段实施路径、验收标准和风险控制。
category: roadmap
created: 2026-05-22
updated: 2026-05-22
...
- `RunBroker + RuntimeEvent + ApprovalRequest`：所有入口共用的协议地基。
...
- EventLog / Trace / Audit：从第一天就是核心能力，不是后补。
...
Quadrant B: Runtime Core
  RunBroker / RuntimeEvent / EventLog / Approval / Trace / Replay
...
```text
B -> A(CLI) -> A(Mobile/Telegram) -> C(read-only) -> C(actions) -> D(team/relay/sandbox)
```

理由：没有 Runtime Core，入口越多越乱；没有 Approval/EventLog，Computer Use 越强越危险。

## 3. 能力价值评估

| 能力 | 价值 | 复杂度 | 优先级 | 结论 |
|------|------|--------|--------|------|
| RunBroker | 极高 | 中 | P0 | 必须先做，承接所有入口和 trace。 |
| CLI | 极高 | 中 | P1 | 开发者闭环与 MCP 桥接基础。 |
| Desktop UI Trace | 高 | 中 | P1 | 让所有外部 run 可见、可复盘。 |
| Mobile approval | 高 | 中高 | P2 | 远程控制的安全体验核心。 |
| Telegram | 中高 | 低中 | P2 | 个人遥控最快 MVP。 |
...
### Phase 1：CLI 与 RunBroker MVP

目标：本机 CLI 能发起、观察、审批和恢复 run。
...
### Phase 2：Run Console 与 Mobile/Telegram MVP
...
### Phase 3：Computer Use 只读 Observation
...
| Trace 后补困难 | 事故后无法复盘 | Phase 0 就定义 EventLog 与 cursor。 |
| Slack 企业复杂度 | OAuth/token/workspace 边界混乱 | Slack 放 Phase 5，先用 Mobile/Telegram 验证模型。 |
...
1. `packages/run-protocol` 类型骨架 + fixtures。
2. `RunBrokerService` interface + file-backed EventLog prototype。
3. `cli-gateway` Pagelet + `telegraph ask/attach/runs`。
4. Desktop Run Console 读取 RunBroker events。

这四步完成后，Mobile、Telegram、Computer Use 都能在同一地基上生长，不会各自发明一套 session。
