...
> 应该都已存在（见 `/Users/ryuyutyo/Documents/code/red/x-oasis/packages/async/async-call-rpc-electron/src/`）。
...
ignores: `**/.vite/**`, `**/dist/**`, `**/node_modules/**`, `**/*.config.{js,ts,mjs}`
...
操作（**等 review 通过我才执行**）：
1. 清理旧代码，创建空的 `apps/telegraph/` 与 `apps/design/` 骨架（package.json + tsconfig.json 起步）
2. 立 `eslint.config.mjs` + `vitest.config.ts` + 根 `package.json` 加 lint/typecheck/test 脚本
3. 立 `apps/telegraph/tsconfig.json` (strict) + `apps/design/tsconfig.json` (strict)
...
- ESLint config 加了若干 ignore：`.agents/**`、`.claude/**`、
...
- IDE LSP 有时会报 stale 错，实际盘上 `tsc --noEmit` 完全干净。**判据以 `pnpm -r typecheck` 为准**，IDE diagnostic 当噪音忽略。
...
直接在 `/Users/ryuyutyo/Documents/code/red/x-oasis/` 仓库里改源码（telegraph 通过
...
- `20260508-port-management-orchestrator-migration-plan.md` — 描述「老 port-manager 与新 orchestrator 共存」（已删除）。
- `20260508-design-only-orchestrator-rewrite-plan.md` — 描述「老代码冷冻 + 新 orchestrator 在原项目里 inplace 重写」（已删除）。
