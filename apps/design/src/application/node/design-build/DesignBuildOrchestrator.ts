...
export interface DesignBuildOrchestratorInput {
  runId: string
  prompt: string
...
export interface DesignBuildOrchestratorOutput {
  brief: DesignBrief
...
export function runDesignBuildOrchestrator(
  input: DesignBuildOrchestratorInput,
...
export function reviewDesignBuildArtifact(artifact: DesignBuildArtifact): DesignBuildReview {
  return reviewArtifact(artifact)
}
...
function reviewArtifact(artifact: DesignBuildArtifact): DesignBuildReview {
  const patchFirst = artifact.kind === 'design-patch'
  const aliasValid = artifact.kind !== 'design-patch' ||
    artifact.operations.every(operation => !operation.content || operation.content.includes("@/packages/ui/"))
  const hasOperations = artifact.kind !== 'design-patch' || artifact.operations.length > 0
  const verdict: DesignBuildReview['verdict'] = patchFirst && aliasValid && hasOperations
    ? 'pass'
    : 'repair_required'
    ...
    checks: [
      {
        ...
        passed: true,
        summary: `Produced structured ${artifact.kind} artifact.`,
      },
      {
        id: 'patch-first',
        passed: patchFirst,
