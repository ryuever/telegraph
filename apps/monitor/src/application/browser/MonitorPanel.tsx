...
import type { MonitorSnapshot, ProcessRow } from '@telegraph/services/connection-orchestrator/common/types'
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        {!snapshot ? (
          ...
          <div className="h-full overflow-auto p-5">
            <Overview snapshot={snapshot} cpuSeries={cpuSeries} memSeries={memSeries} />
          </div>
        ) : tab === 'processes' ? (
          ...
          <div className="h-full overflow-auto pb-2">
            <PsTreePanel tree={snapshot.pidTree} query={query} />
          </div>
        )}
      </div>
...
function SegmentedTabs({ value, onChange }: { value: TabId; onChange: (v: TabId) => void }) {
  return (
    <div
      className="flex items-center gap-0.5 rounded-md border border-zinc-800/80 bg-zinc-900/70 p-0.5 backdrop-blur"
...
function Header({
  ...
  const ago = updatedAt != null ? Math.max(0, Math.round((Date.now() - updatedAt) / 1000)) : null
  return (
    <header className="flex items-center justify-between gap-3 border-b border-zinc-800/80 bg-zinc-900/40 px-5 py-3">
      <div className="flex items-baseline gap-3">
        <h1 className="text-base font-semibold tracking-tight text-zinc-100">Activity</h1>
        <span className="text-[11px] text-zinc-500">
          {snapshot ? `${String(snapshot.processes.length)} processes` : 'connecting…'}
        </span>
      </div>
...
function Overview({
  ...
  const topCpu = useMemo(
    () => snapshot.processes.slice().sort((a, b) => b.cpu - a.cpu).slice(0, 5),
    [snapshot.processes],
  )
  const topMem = useMemo(
    () => snapshot.processes.slice().sort((a, b) => b.memory - a.memory).slice(0, 5),
    [snapshot.processes],
  )
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        >
          <div className={cn('mt-3', cpuColor)}>
            <Sparkline values={cpuSeries} max={Math.max(100, ...cpuSeries)} height={56} />
          </div>
        </StatCard>
        <StatCard
          label="Memory"
        ...
        >
          <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-zinc-400">
            <MiniStat label="renderers" value={countByType(snapshot.processes, 'Tab')} />
            <MiniStat label="utility" value={countByType(snapshot.processes, 'Utility')} />
            <MiniStat label="gpu" value={countByType(snapshot.processes, 'GPU')} />
  ...
  return (
    <div className="rounded-2xl border border-zinc-800/80 bg-gradient-to-b from-zinc-900/80 to-zinc-900/40 p-4 shadow-sm">
      ...
      <div className="mt-1 flex items-baseline gap-1.5">
        <span className={cn('font-mono text-3xl font-semibold tabular-nums', accentClass)}>
          {value}
        </span>
...
function countOther(processes: ProcessRow[]) {
  return processes.filter((p) => !['Tab', 'Utility', 'GPU'].includes(p.type)).length
}
            ...
            <li key={p.pid} className="space-y-1">
              <div className="flex items-center justify-between gap-2 text-[12px]">
                <span className="flex min-w-0 items-center gap-2">
                  <span className="truncate text-zinc-200" title={p.name ?? p.type}>
                    {p.name ?? p.type}
                  </span>
...
function Footer({ snapshot }: { snapshot: MonitorSnapshot | null }) {
  return (
    <footer className="flex items-center gap-5 border-t border-zinc-800/80 bg-zinc-950/80 px-5 py-2 text-[11px] backdrop-blur">
      <FooterStat label="CPU" value={snapshot ? `${snapshot.totals.cpu.toFixed(1)}%` : '—'} />
      <FooterStat label="Mem" value={snapshot ? `${snapshot.totals.memory.toFixed(0)} MB` : '—'} />
      <FooterStat label="Procs" value={snapshot ? String(snapshot.processes.length) : '—'} />
    ...
    <div className="flex h-full items-center justify-center">
      <div className="text-center">
        ...
        <p className="mt-1 text-[11px] text-zinc-500">
          The daemon ticks every 5 seconds.
        </p>
      </div>
    </div>
  )
