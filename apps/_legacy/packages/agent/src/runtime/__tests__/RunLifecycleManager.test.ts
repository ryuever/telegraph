...
function deepEqual(actual: any, expected: any, message?: string): asserts actual is typeof expected {
...
function assertThrows(fn: () => void, messagePattern?: string | RegExp, message?: string) {
  try {
    fn()
    throw new Error(`Expected function to throw, but it did not. ${message || ''}`)
  } catch (e) {
    if ((e as Error).message.includes('Expected function to throw')) throw e
    if (messagePattern) {
...
export function runTests() {
  ...
  test('should throw if markRunning called twice', () => {
    ...
    assertThrows(
      () => manager.markRunning(),
      "Cannot mark running: already in state 'running'"
    )
  })
  ...
  test('should throw if called from initial state', () => {
    ...
    const fallback = { code: 'timeout', message: 'Timeout reached' }
    assertThrows(
      () => manager.ensureTerminal(fallback),
      "Cannot ensure terminal: invalid state 'initial'"
    )
  })
