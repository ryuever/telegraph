...
import { BaseAgentRuntime } from './AgentRuntime';
...
import { ToolExecutor } from './toolExecution/ToolExecutor';
...
export class PiEmbeddedRuntime extends BaseAgentRuntime {
    id = 'pi-embedded';
    label = 'Pi Embedded (In-Process + Tools)';
    sessionStore;
    toolRegistry;
    toolExecutor;
        ...
        try {
            ...
            while (shouldContinue && loopIteration < this.maxToolIterations) {
                ...
                for await (const event of streamPiAiRuntimeEvents({
                    ...
                    if (event.type === 'model_event' && event.data?.content) {
     ...
     */
    async executeToolCalls(runId, parsedCalls, session) {
