...
export class BaseHarness {
  ...
  setSettings(next: AgentRuntimeSettings) {
  ...
  appendMessage(msg: AgentTextMessage) {
   ...
   * is obvious.
   */
  async run({ userMessage, signal }: HarnessRunInput, handlers: HarnessRunHandlers = {}): Promise<string> {
