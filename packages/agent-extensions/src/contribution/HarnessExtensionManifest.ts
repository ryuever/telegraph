/**
 * Declarative (manifest-driven) extension contribution model.
 *
 * Migrated from `@/packages/agent-extension-host` as part of D-016 P5 to live
 * alongside the new command-style ExtensionHost loader. Both forms now share
 * `@/packages/agent-extensions` as their owning package; the old
 * `agent-extension-host` package keeps re-export shims for one release before
 * being deleted in P6.
 *
 * The manifest schema here is the *legacy* harness-extension shape consumed by
 * `ContributionRegistry`. New command-style extensions use the much smaller
 * `ExtensionManifest` defined in `../manifest.ts`.
 */
