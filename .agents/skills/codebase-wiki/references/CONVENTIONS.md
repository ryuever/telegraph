...
### 3.7 Roadmap 文档建议章节（推荐）

`roadmap/` 文档默认采用“决策 + 能力评估 + 分阶段清单 + 验收/No-Go”的实施路线图风格。
详细模板见 `references/ROADMAP-CHECKLIST.md`。生成时优先保持这些结构特征：

1. `## 决策摘要`：用“优先押注 / 谨慎推进 / 明确不做”先收束判断。
2. `## 技术象限` 或 `## 能力地图`：用简短文本图说明能力分组、层次或推进顺序。
3. `## 能力价值评估`：用表格列出能力、价值、复杂度、优先级、结论。
4. `## 分阶段路线`：每个 Phase 必须包含目标、状态、交付、验收、No-Go。
5. `## 后续 TODO 清单`：按领域分组，使用 Markdown checkbox，粒度保持可执行。
6. `## Repo 落点建议`：实现型 roadmap 应给出目录、模块或 PR 边界。
7. `## 风险清单`：用“风险 / 表现 / 缓解”表格提前约束失败模式。
8. `## 第一批 PR 建议`：列出 3-7 个可独立 review 的切片。
9. `## 长期判断`：收束战略判断，避免路线图只剩任务堆叠。

Roadmap 的 checkbox 只用于真实交付项：已验证完成才标记 `[x]`，未来工作使用 `[ ]`。
部分完成不要发明半选状态，应写入该 Phase 的 `状态` 段。
