...
// ❌ 全仓库禁止
import { ... } from '../../_legacy/...'
import { ... } from 'apps/_legacy/...'
```

`apps/_legacy/` 是历史档案。runtime / extension / trace 的旧实现都在那里，**不准 import、不准复制粘贴改名、不准"参考着写"地直接搬过来**。允许的用法只有：阅读它来理解历史决策。

### 2.3 RuntimeEvent 协议红线（A-005 §4.2 / §15.2）
...
| `packages/agent-runtime/`（PiAiRuntime / PiCliRuntime / ...） | ❌ 不存在；旧版在 `apps/_legacy/packages/agent/src/runtime/` |
| `AgentStreamService` / `RuntimeEventForwarder` | ❌ 不存在；旧版在 `apps/_legacy/telegraph/src/services/agent/` |
| `LlmTracePanel` / `RuntimeTimeline` | ❌ 不存在；旧版在 `apps/_legacy/packages/ui/src/components/chat/` |
| pi-subagents adapter | ❌ 不存在；旧版在 `apps/_legacy/packages/agent/src/runtime/piSubagents/` |
...
| `apps/_legacy/` | 历史实现（仅供阅读） | 想知道"上一版怎么解决 X"时 |
