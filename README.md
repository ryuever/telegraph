...
Telegraph is an Electron + React + Vite desktop workspace for exploring multi-process AI app architecture. It is built as a pnpm monorepo with a main Electron app, pagelet-style feature apps, shared UI packages, and an agent protocol layer that keeps agent events and cross-process boundaries explicit.
...
- **Shared packages:** `packages/ui`, `packages/services`, `packages/agent-protocol`, `packages/stores`, and `packages/agent` keep UI, service contracts, agent protocol, state, and runtime foundations reusable.
